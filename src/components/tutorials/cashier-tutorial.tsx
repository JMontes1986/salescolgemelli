...
function TutorialHeader({ variant }: { variant: "dashboard" | "public" }) {
  if (variant === "dashboard") {
    return (
      <PageHeader
        title="Tutorial para Cajas"
        description="Guía rápida para vender, procesar devoluciones y validar compras de autogestión."
      />
    );
  }

  return (
    <div className="flex w-full flex-col gap-5 border-b pb-6 md:flex-row md:items-end md:justify-between">
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-sm leading-6 text-muted-foreground sm:text-base">
            Guía rápida para revisar antes de iniciar sesión: cómo vender, registrar devoluciones y validar compras de autogestión.
          </p>
        </div>
      </div>
      <Button asChild>
        ...
        <CardHeader>
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {quickLinks.map((item) => (
                <Button key={`${item.href}-${item.label}`} asChild variant="outline" className="justify-start bg-background">
                  <Link href={item.href}>
